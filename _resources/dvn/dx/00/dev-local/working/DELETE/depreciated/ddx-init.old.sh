...
function InstallPrerequisites() {
	clear
	# don't prompt
	sudo apt-get install dkms build-essential localepurge linux-headers-$(uname -r) |& tee $HOME/DevenDX/Logs/prerequisites.log
}
...
function InstallCLIOptional(){
		sudo apt-get install emacs zip unzip curl |& tee $HOME/DevenDX/Logs/cli-optional.log

}

function InstallGUI(){
		sudo apt-get install xserver-xorg xserver-xorg-core xfonts-base xinit x11-xserver-utils xfce4 xfce4-terminal lightdm lightdm-gtk-greeter |& tee $HOME/DevenDX/Logs/gui-install.log
		pushd $HOME/DevenDX/Resources
		sudo ./VBoxLinuxAdditions.run
		popd
...
function InstallGUIOptional(){
		sudo apt-get install iceweasel |& tee $HOME/DevenDX/Logs/gui-optional.log
}

function InstallPython3(){
		sudo apt-get install python3 python3-dev |& tee $HOME/DevenDX/Logs/python3.log
}
...
if [ $disableSuggestedPackages = "true" ]; then
...
if [ $installVBoxTools = "true" ]; then
	InstallVBoxTools
fi

if [ $installCLIOptional = "true" ]; then
	InstallCLIOptional
fi
...
if [ $installGUIOptional = "true" ]; then
	InstallGUIOptional
fi

if [ $installPython3 = "true" ]; then
	InstallPython3
fi

#if [ $installVirtualenv = "true" ]; then
#	InstallVirtualEnv
#fi

if [ $installRVM = "true" ]; then
	#InstallRVM
fi

if [ $installVSCode = "true" ]; then
	#InstallVSCode
fi

if [ $createMOTD = "true" ]; then
	CreateMOTD
fi
